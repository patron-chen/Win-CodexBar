...
describe("MenuCard", () => {
  ...
  it("renders local token and cost totals after chart data loads", async () => {
    ...
    const localSummary = container.querySelector(".menu-card__local-usage")!;
    const details = container.querySelector<HTMLDetailsElement>(".menu-card__more")!;
    expect(details.contains(localSummary)).toBe(false);
    expect(
      localSummary.compareDocumentPosition(details) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(details.contains(screen.getByText("Estimated from local logs"))).toBe(true);
    expect(details.open).toBe(false);
    fireEvent.click(details.querySelector("summary")!);
    expect(details.open).toBe(true);
    fireEvent.click(details.querySelector("summary")!);
    expect(details.open).toBe(false);
    expect(screen.getByText("584K")).toBeVisible();
  });
