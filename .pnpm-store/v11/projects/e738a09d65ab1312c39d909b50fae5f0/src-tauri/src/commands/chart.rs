...
use codexbar::core::{CostScanOptions, OpenAIDashboardCacheStore};
...
fn build_provider_chart_data_with_cancel(
        ...
        .is_some_and(|flag| flag.load(Ordering::Relaxed))
    {
        ...
        // `provider.updatedAt` drives this command after each provider refresh.
        // Do not serve the prior 30-second summary here: it can predate a
        // rollout that just grew and would keep Today/Latest visibly stale.
...
fn load_local_usage_summary_with_unknown_models(
    ...
    // This path is the desktop's explicit usage refresh. Force the wide scan
    // so a rollout that grew inside the scanner debounce window is observed;
    // the one-day summary below can then reuse that freshly updated cache.
    let Some(thirty_day) = scan_local_cost(provider_id, 30, cancel, CostScanOptions::app_driven())
    else {
    ...
    let today =
        scan_local_cost(provider_id, 1, cancel, CostScanOptions::default()).unwrap_or_default();
...
fn scan_local_cost(
    ...
    let scanner = CostScanner::new(days).with_options(options);
    match provider_id {
